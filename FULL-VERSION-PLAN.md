...
2026-07-28 fresh Samsung vertical run 已用 74 份重建 Clip Cards、實際音樂與新的 direct-video plan 產生 84.17 秒、1080×1920 的 music-only audition；picture run 為 22 次付費 interaction、估算 US$0.384894，低於 25 次上限。Strict delivery 正確阻擋，因 watch UI／closing reaction exact event 在 selected windows 中 unresolved、AI payoff cue 差 16 frames，且兩章未授權重用同一 source interval。這次暴露的 runtime 問題已改為：空 exact-event selection 持久化為 fail-closed unresolved artifact，不做 schema 重送；已可判定的 source reuse 在 grounding／render 前換 Top-K；成功候選會產生 resolved ledger 並 supersede 舊 exhaustion artifact。尚待完成的是 exact-event unresolved 自動換 Top-K、cue-aware trim reconciliation、16:9 runtime exact-event integration、coarse/dense 統一 review UI、SAM 週期語意重驗、遮擋後 re-identification、overlay layout solver 與三次穩定度報告。本文件同時包含已實作與後續設計；任何缺少對應 authority 的建議都不得當成 production cut 或 SpatialTrack。
