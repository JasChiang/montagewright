...
Samsung 9:16 selected-window evidence experiment 曾執行 23 次付費 interaction，並產生六個 ExactEventLocks 與完整 bundle；事後稽核確認它明確重用了舊 `direct-video-edit-plan-v2` 與舊 picture artifacts，輸出音軌也全是 source audio，delivery 在 music mux 前即停止。它只能證明 selected-window evidence integration，不能算 fresh autonomous edit benchmark，相關 run 已移至 `artifacts/_archive/2026-07-28-stale-plan-reuse/`。Autonomous profile 現在會在付費前拒絕兩個 feature-plan reuse flags；Samsung strict policy 也禁止 solid matte fit，無法滿版時必須換 Top-K 或 block。尚待執行的是全新 output namespace、全新 music-aware plan、實際音樂 audition mux 與 cue-aware trim／duration reconciliation 的正式 benchmark；另外仍有長毛片自動 take segmentation／重拍分組、16:9 runtime exact-event integration、coarse/dense 統一 review UI、SAM 週期語意重驗、遮擋後 re-identification、overlay layout solver 與三次穩定度報告。本文件同時包含已實作與後續設計；任何缺少對應 authority 的建議都不得當成 production cut 或 SpatialTrack。
