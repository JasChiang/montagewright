...
def review_cut(
    ...
    *,
    ...
    interaction = client.interactions.create(
        ...
        generation_config={"thinking_level": "high", "max_output_tokens": 8192},
        response_format={
...
def adjudicate(
    degradations: list[DegradationStep], verdict: ReviewVerdict
) -> list[DegradationStep]:
    """Settle each degradation against what the reviewer saw.

    A degradation the reviewer did not mention is accepted: they watched the
    cut and it did not trouble them. One they did mention goes back for a
    replan, because the way to answer a bad fallback is a different plan, not
    a better fallback. Anything left unsettled is labelled as such rather than
    quietly shipped.
    """
